...
export const registerSlot = async (req: Request, res: Response) => {
  try {
    ...
    slot.registeredUsers.push(user._id as Types.ObjectId);
    await slot.save();
    await slot.save();

    if (dose === 1) user.firstDoseSlot = slot._id as Types.ObjectId;
    else user.secondDoseSlot = slot._id as Types.ObjectId;
...
export const updateSlot = async (req: Request, res: Response) => {
  try {
    ...
    oldSlot.registeredUsers = oldSlot.registeredUsers.filter((id: Types.ObjectId) => !id.equals(user._id));
    await oldSlot.save();
    newSlot.registeredUsers.push(user._id as Types.ObjectId);
    await newSlot.save();
    await newSlot.save();

    if (dose === 1) user.firstDoseSlot = newSlot._id as Types.ObjectId;
    else user.secondDoseSlot = newSlot._id as Types.ObjectId;
